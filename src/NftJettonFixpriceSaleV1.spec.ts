...
>([
  ...
  [
    randomAddress(),
    {
      fullPrice: toNano(1.2),
      marketplaceFee: toNano(0.12),
      royaltyAmount: toNano(0.12),
    },
  ],
  [
    randomAddress(),
    {
      ...
      marketplaceFee: toNano(0.11),
      royaltyAmount: toNano(0.11),
    },
  ],
]);
...
const defaultConfig: NftJettonFixpriceSaleV1Data = {
  ...
  publicKey: keyPairFromSeed(new Buffer(32)).publicKey,
};
...
describe("fix price jetton sell contract v1", () => {
  ...
  it("should return sale info", async () => {
    ...
    defaultConfig.publicKey = keypair.publicKey;
    ...
    const deployMsg = beginCell()
      .storeUint(5, 32)
      .storeUint(0, 64)
      .storeBuffer(sign(sale.contract.dataCell.hash(), keypair.secretKey))
      .endCell();

    let r = await sale.contract.sendInternalMessage(
      new InternalMessage({
        to: sale.address,
        ...
        body: new CommonMessageInfo({
          body: new CellMessage(deployMsg),
        }),
      })
    );
    ...
    expect(res.marketplaceAddress.toFriendly()).toEqual(
      defaultConfig.marketplaceAddress.toFriendly()
    );
    expect(res.nftAddress.toFriendly()).toEqual(
      defaultConfig.nftAddress.toFriendly()
    );
    expect(res.nftOwnerAddress?.toFriendly()).toEqual(
      defaultConfig.nftOwnerAddress!.toFriendly()
    );
    expect(res.marketplaceFeeAddress.toFriendly()).toEqual(
      defaultConfig.marketplaceFeeAddress.toFriendly()
    );
    expect(res.royaltyAddress.toFriendly()).toEqual(
      defaultConfig.royaltyAddress.toFriendly()
    );
    ...
    for (let key of jettons.keys()) {
      ...
      let b = res.jettonPrices.get(
        [...res.jettonPrices.keys()].find(
          (k) => k.toFriendly() === key.toFriendly()
        )!
      ); // Map is missbehaving with Address keys
      assertNotNull(a);
      assertNotNull(b);
  ...
  it.each([
    ...
    "should not buy if not initialized",
    async (config: NftJettonFixpriceSaleV1Data) => {
      const keypair = keyPairFromSeed(await getSecureRandomBytes(32));
      defaultConfig.publicKey = keypair.publicKey;
    ...
    const nftTransfer = res.actionList.find((tx) => {
      if (tx.type === "send_msg") {
        if (tx.message.info.type === "internal") {
          if (
            tx.message.info.dest?.toFriendly() ===
            defaultConfig.nftAddress.toFriendly()
          ) {
            ...
            if (
              ...
              newOwner?.equals(buyerAddress) &&
              forward.gte(toNano("0.03"))
            ) {
              return true;
            }
            return false;
      ...
      if (tx.type === "send_msg") {
        if (tx.message.info.type === "internal") {
          if (
            ...
            return tx.message.info.value.coins.gte(
              toNano(2)
                .sub(defaultConfig.marketplaceFee)
                .sub(defaultConfig.royaltyAmount)
                .sub(toNano(1))
            );
          }
        }
  ...
  it("should buy only for allowed jettons", async () => {
    ...
    for (let [jettonAddress, prices] of jettons) {
      ...
      // Owner revenue
      {
        ...
        assertCoins(
          slice.readCoins(),
          prices.fullPrice.sub(prices.marketplaceFee).sub(prices.royaltyAmount)
        ); // amount
      ...
      // Nft transfer
      {
        ...
        assertAddress(
          res.actionList[3].message.info.dest,
          defaultConfig.nftAddress
        );
  ...
  it.each([
    ...
    "should bounce jettons if not initialized",
    async (config: NftJettonFixpriceSaleV1Data) => {
      const keypair = keyPairFromSeed(await getSecureRandomBytes(32));
      defaultConfig.publicKey = keypair.publicKey;
      const buyerAddress = randomAddress();
  ...
  it("should return extra jettons", async () => {
    ...
    // Nft transfer
    {
      ...
      assertAddress(
        res.actionList[4].message.info.dest,
        defaultConfig.nftAddress
      );
      ...
      if (tx.type === "send_msg") {
        if (tx.message.info.type === "internal") {
          if (
            ...
            if (
              op.eq(new BN(0x5fcc3d14)) &&
              newOwner?.equals(defaultConfig.nftOwnerAddress!)
            ) {
              return true;
            }
            return false;
